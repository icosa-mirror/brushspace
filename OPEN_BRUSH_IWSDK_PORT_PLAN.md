...
- Phase 10 foundation has progressed: the consolidated `welcome` panel is now explicitly the browser/debug fallback. In browser/non-immersive mode it remains the fallback `ScreenSpace` panel; in XR it is hidden by `PanelAttachmentSystem`, while lightweight `Color`, `Brush`, and `Tools` `PanelUI` entities form a fixed off-hand/wand ring. The ring consumes `SettingsState` (`dominantHand`, `panelAnchor`, `panelDistance`, `panelHeight`, `panelScale`, `wandPanelRotationSteps`), attaches to the requested hand ray or XR-origin anchor, applies panel sizing through `PanelUI.maxWidth/maxHeight`, exposes `OpenBrushPanelAttachment` role/mode/slot/visibility fields for runtime inspection, mirrors across handedness swaps, and rotates by cached slot steps. Phase A now treats that ring as the primary XR UX and trims it to reliable MVP actions only: Draw, Line/Straightedge, Eraser, Color Picker, Brush Picker, Dropper, Undo/Redo, Brush Prev/Next/Size/Radius, color swatches, and current-color feedback. Mirror/Grid/Lazy/Tape/Stencil remain implementation experiments, not first-class Phase A hand-panel actions, until their reference interaction semantics are real. The configured Wand hand can also rotate the fixed ring with horizontal thumbstick deflections; the edge detector only applies one rotation per deflection and respects `xrRayEnabled`. This is still not full UI parity: direct touch/paging/popups, full Open Brush palette panels, reference-specific advanced tool visuals/semantics, and floating/alternate panels remain pending.
...
- Managed runtime checks on browser generation 355 confirmed panel-focus feedback for the user-reported "eraser does not work" route. With `activeTool=eraser`, the Brush-hand ray aimed at a visible hand-attached Brush panel and trigger held, `OpenBrushAppState.toolStatus` changed to `erase-panel-focus`, `OpenBrushEraserCursor.visible/hot` stayed true for cursor feedback, and no stroke entities or history changes were created. Moving the Brush hand clear of the panel while the trigger remained held cleared the temporary status back to `erase-ready`; fresh post-reload warn/error console logs were empty. The hand-attached Brush panel now labels the blocked state as `panel focus` so panel suppression is visible instead of looking like a dead Eraser/Draw action.
- Managed runtime checks on browser generation 409 confirmed the picker family after the picker-hit diagnostics landed. Color Picker changed only `BrushSettings.color` from a live Light stroke while preserving the current Marker brush and normalized size. Brush Picker changed only the active brush identity to Light, preserved the current color and `size01`, and recalculated absolute size for the picked brush range. Dropper changed brush, color, and size from the same stroke, restoring Light/blue/`0.002353189...` after starting from Marker/red/different size. Fresh warnings/errors remained empty in the managed console.
- Managed runtime checks on browser generation 409 also confirmed the current Phase A hand-panel visual states without restarting the dev server. `npx tsc --noEmit`, targeted Phase A Vitest coverage, `git diff --check`, and `npm run build` passed; the build still emits only the existing large-chunk warning. XR screenshots now cover Eraser radius mode, panel-focus blocking, successful eraser hit, and eraser miss. ECS inspection confirmed the Eraser cursor is visible at radius `0.2` with forward offset `0.05`; panel focus reports `erase-panel-focus` with no stroke/history change; a hit reports `erased 1 stroke`, hides the stroke, reduces visible stroke count to zero, and advances undo depth; a miss reports `nothing-to-erase` with undo depth unchanged. A handedness shortcut check flipped `SettingsState.dominantHand` and verified the off-hand panel cluster mirrors to the opposite hand with roles, slots, and visibility intact. Unit coverage now fixes `isOpenBrushPanelFocusable` so hidden fallback panels and zero-size panels cannot regress into blocking Draw/Eraser/Picker.
- A final visible-route pass on browser generation 409 selected the remaining Phase A hand-panel controls by their named scene Object3Ds. The Color panel's `color-yellow` swatch changed only `BrushSettings.color` to yellow. The Brush panel's `brush-next` and `brush-prev` buttons cycled Light -> Marker -> Light and recalculated absolute size for each brush's range while preserving `size01`; `brush-size-up` changed `size01` from `0.25` to `0.30` and updated the Light live size to `0.0017675067`. The Tools panel's `tool-line` button kept `activeTool=free-paint`, set `straightEdgeEnabled=true`, and a Brush-hand stroke then produced a finalized `toolId=straightedge` stroke with 31 control points at the active size and color. The `stroke-history-undo` and `stroke-history-redo` buttons hid and restored that stroke with the expected undo/redo depths and request revisions, and `tool-draw` returned the app to `straightEdgeEnabled=false` / `toolStatus=draw-ready`. A managed screenshot captured the yellow swatch, Draw-highlighted tools panel, readable Brush panel, and restored straightedge stroke in the same XR view.
- Phase A unit coverage now locks the compact Brush panel label contract for active brush metadata, ready warnings, Size -/+ versus Radius -/+ controls, adjusted Eraser radius percentages such as `Radius 55% | 0.210 m`, panel-focus wording, hit/miss feedback, picker/dropper guidance, and invalid-size fallback copy. `npx tsc --noEmit`, `git diff --check`, and the focused Phase A Vitest suite passed with 97 tests after this coverage was added.
- Managed runtime checks on browser generation 411 confirmed the fixed-ring panel placement update after hot reload without restarting the dev server. The lower Brush and Tools slots now resolve to local `y=0.10` instead of the previous `y=-0.02`, while the Color slot remains at `y=0.34`; `OpenBrushPanelAttachment` still reports visible off-hand fixed-ring panels with the same roles and slot indices. A managed screenshot shows the Tools button grid clear of the controller/hand visual, and fresh warn/error console logs after a post-check cutoff were empty. `npx tsc --noEmit`, `git diff --check`, the focused Phase A Vitest suite, and `npm run build` passed; the build still emits only the existing large-chunk warning.
- Managed runtime ECS shortcut checks on browser generation 411 also verified Phase A anchor safety after the visible UI route was already proven. Setting `SettingsState.panelAnchor=center` moved Color/Brush/Tools to `hand=none`, `status=xr-center`, parented to the XR/player origin, and kept all three panels visible with fixed-ring roles and slots. Setting `panelAnchor=dominant-hand` moved the ring to the right-hand ray with mirrored horizontal offsets and the same lifted lower-slot `y=0.10`; restoring `panelAnchor=off-hand` returned Tools to the left-hand ray at `slotIndex=2`, `slotAngleDegrees=240`, and `visible=true`.
- A follow-up managed runtime visible-route pass on browser generation 411 completed the remaining Phase A hand-panel interactions without restarting the dev server. Brush Picker sampled the existing Light/yellow straightedge stroke and changed Marker -> Light while preserving the current red color and `size01=0.55`; Dropper was then selected from the Tools panel, first produced `nothing-to-pick` miss feedback at deliberately wrong offset poses, and then, with the live grip-space forward-offset math applied, changed Marker/red/`0.0232042819` to Light/yellow/`0.0025126834` from stroke #1. Selecting Eraser from the hand-attached Tools panel made the grip-parented cursor visible with `radius=0.2` and `forwardOffset=0.05`; a miss reported `nothing-to-erase` with both strokes visible and undo depth unchanged at `2`; a Brush-panel focus hold reported `erase-panel-focus`, kept the cursor hot, and left stroke/history state unchanged; a stroke hit reported `erased 1 stroke`, hid stroke #1, reduced `PerformanceState.visibleStrokeCount` from `2` to `1`, and advanced `StrokeHistoryState.undoDepth` from `2` to `3`. An unfiltered managed console read showed only IWSDK MCP trace logs for the interaction window, and the supplemental warn/error query was empty.
- A read-only ultracode sidecar audit of the upstream Open Brush tool specs confirmed that Draw/Line startup thickness is not currently explained by a wrong normalized default: upstream Light starts at `BrushSize01=0.5`, raw size `0.1125`, and Line/Straightedge intentionally forces pressure to `1`. The remaining thickness risk is the IWSDK live-unit calibration and simplified ribbon/tube renderer versus Open Brush brush prefabs/shaders. The same audit identified Eraser reliability coverage as the next code slice; `StrokeAuthoringSystem` now delegates Eraser hit decisions to a pure helper that tests active-layer/finalized/visible gates and generated-geometry hit precedence before fallback AABB bounds. Remaining Eraser reliability work is a fuller system/pose regression that proves cursor center and erase center share the same controller-forward convention.
- Eraser cursor/erase-center pose math is now centralized in a shared `tool-pose` helper. Unit coverage fixes the Brush-tool local forward convention as `-Z`, verifies world offset rotation for a known controller yaw, and keeps the visible cursor local offset and stroke-authoring erase/picker center on the same calculation. Managed runtime checks on browser generation 360, after hot reload without restarting the dev server, set Eraser active with a known right-controller orientation and confirmed `OpenBrushEraserCursor` remained visible on the right grip with `Transform.position=[0,0,-0.05]`, `radius=0.2`, and no fresh warn/error console logs.
- Managed runtime checks on browser generation 366 confirmed the hand-attached Tools panel could wire Mirror, Grid, Lazy, Tape, and Stencil in addition to Draw/Line/Eraser/Color/Brush/Dropper/Undo/Redo. That result is retained as a foundation check, but Phase A no longer exposes those advanced placeholders in the primary Tools hand panel. The wand eraser selector is labeled `Eraser` to distinguish the held eraser tool from the browser/debug fallback's one-shot `Erase Target` command.
- The renewed tool-spec audit remains active. The latest sidecar pass confirmed upstream Draw uses trigger ratio every frame, while IWSDK `InputCommandSystem` previously fell back to pressure `1` for XR pressed-without-value, keyboard paint, and browser pointer paint. Input pressure fallback is now explicit and tested: analog trigger/pointer values pass through unchanged, unpressed inputs resolve to `0`, and digital/no-analog paint input falls back to midpoint pressure `0.5` instead of full pressure. Managed runtime checks on browser generation 368 set the right XR trigger to `0.2` and confirmed `InputCommandState.pressure=0.2000000029` with fresh warn/error logs empty. Straightedge/Line still intentionally matches upstream `LineCreator` pressure `1`, so a full-pressure Draw or Line can still look thicker than a light analog Draw stroke even with the correct `BrushSize01=0.5`. Next thickness work should compare rendered width visually under equal pressure assumptions and recalibrate live render width only if the IWSDK geometry still reads too thick at the intended world scale.
- The current Draw/Line thickness queue now includes the upstream brush-opacity side of pressure, not only pressure-size. The source Light brush uses `m_PressureOpacityRange=[0.5,1]`; the port carries mapped pressure-opacity ranges in `BrushInventoryEntry`, feeds them into live runtime geometry and GLB export, and unit-tests that a half-pressure Light-style stroke produces `0.75` vertex alpha. This is not the end of visual width calibration: rendered apparent thickness still needs equal-pressure XR screenshots, Open Brush prefab/shader comparison, and line/full-pressure labeling. Eraser feedback is also being tightened: erase misses now set a forced `nothing-to-erase` status and hits report the erased stroke count, so the UI can distinguish a true miss from panel focus or a broken tool route.
- Managed runtime checks on browser generation 379 validated the current feedback slice after hot reload without restarting the dev server. Startup still reported the upstream Light brush at `BrushSettings.size01=0.5` and `size=0.002353189...`; a right-controller Draw stroke finalized with `BrushStroke.brushSize=0.002353189...`, `controlPointCount=3`, and one visible stroke. With Eraser active, pressing far from the stroke set `OpenBrushAppState.toolStatus=nothing-to-erase` without changing stroke visibility or undo depth. Pressing on the stroke then changed `toolStatus` to `erased 1 stroke`, changed `BrushStroke.visible/renderVisible` to `false`, reduced visible stroke counts to zero, advanced undo depth from `1` to `2`, and produced no fresh warn/error console logs. The hit status is now hold-scoped so the next empty eraser frame does not immediately overwrite it.
- Managed runtime checks on browser generation 391 confirmed the replanned Phase A hand-panel surface after hot reload without restarting the dev server. The visible Tools panel contains only Draw, Line, Eraser, Color Pick, Brush Pick, Dropper, Undo, and Redo; selecting Eraser through the hand-attached Tools panel changed `OpenBrushAppState.activeTool` to `eraser`, showed the grip-parented `OpenBrushEraserCursor` with radius `0.2`, highlighted Eraser, dimmed empty Undo/Redo, and relabeled the hand-attached Brush panel controls to `Radius -` and `Radius +` with an Eraser radius readout. Fresh warn/error console logs were empty, and the visual checkpoint framed the three-panel off-hand/wand layout in XR.
...
- Hand-attached UI: core brush/color/tool controls belong on off-hand/wand-attached spatial panels with brush/wand role semantics. The consolidated panel remains a browser/debug fallback, not the target XR UX. The current Phase A XR wand panels cover the trustworthy local painting loop: Draw/Line/Eraser, Color Picker, Brush Picker, Dropper, stroke undo/redo requests, brush prev/next/size, Eraser radius labels/readout when Eraser is active, a few color swatches, current-color feedback, active tool/mode highlighting, and disabled Undo/Redo feedback. Missing XR controls include dropper image-widget sampling, layers, selection, settings, save/load/export/playback, paging/popups, direct touch, richer palette controls, and the reference-specific visuals/semantics behind the parked Mirror/Grid/Lazy/Tape/Stencil descriptors.
...
Phase A stop definition:

- Stop after Phase A when the local XR painting loop is complete through the hand-attached panels: Draw, Line/Straightedge, Eraser, Color Picker, Brush Picker, Dropper, brush prev/next, size/radius, swatches, Undo, and Redo.
- Treat hand-panel feel as a release blocker. Phase A is not complete if the controls technically work but still read as a cramped debug ring, obscure the hand/controller, overlap each other, have ambiguous active states, or require the browser fallback for normal painting.
- Require managed `iwsdk-runtime` evidence, not direct Playwright: ECS/scene inspection, console-log checks without level filtering, and XR screenshots that frame the default panel cluster, close-up readability, Eraser radius mode, panel-focus blocking, miss feedback, and a successful draw/erase workflow.
- Do not start multiplayer, cloud storage, cloud-powered sharing, account-backed services, or networked collaboration. Those are out of scope for this port plan, not Phase A backlog.
...
- **A1: Redesign the hand-panel surface until it is the product, not a debug panel.**
  Scope: Make the hand panels the first-class Phase A product surface. Follow the Open Brush/Tilt Brush brush-and-wand model: the Brush hand paints, erases, and picks from the grip/tool pose, while the off-hand/wand role carries compact reachable controls. Replace the current clunky ring/debug-panel feel with a composed set of small world-space `PanelUI` panels: Tools, Brush, and Color. Use `PanelUI.maxWidth/maxHeight` for stable dimensions, `OpenBrushPanelAttachment` for runtime-inspectable role/slot/visibility state, and hand/wand role settings for handedness-safe mirroring. Tune panel distance, height, scale, slot lift, button density, text size, and panel ordering together so the controls sit in a comfortable reachable cluster instead of a flat crowded board.
  Acceptance: The first XR view has a clean workspace plus reachable hand-attached Tools/Brush/Color panels. Controls are visibly grouped, target sizes are comfortable for ray selection, text does not overflow, button states are readable, and the layout still works after dominant-hand swaps or panel-anchor changes. The panels do not obscure the Brush hand, sit clear of the controller/hand model, avoid vertical stacking that buries lower buttons, and leave enough negative space that Tools, Brush, and Color read as separate control surfaces. There is no giant all-in-one panel, no cramped placeholder grid, and no primary control surface that reads as a debug overlay. The browser fallback is still functional but is clearly a fallback/debug surface.
  Testing: Use `xr_get_session_status` first and keep the managed dev server alive if connected. Enter XR only as needed, query `PanelUI`, `PanelDocument`, `Transform`, and `OpenBrushPanelAttachment`, then capture managed `browser_screenshot` views of the default layout, handedness mirror, and each panel close enough to inspect readability. Query panel transforms to confirm the lower Tools/Brush slots are lifted clear of the hand/controller. Do not sign off Phase A until these screenshots show the hand panels are readable and comfortable.
...
- **A5: Phase A signoff and stop.**
  Scope: Stop after Phase A unless scope is explicitly promoted. Later phases remain roadmap context only. Do not begin full layer/save/load/export, account-backed, cloud-adjacent, or networked work as part of the Phase A push.
...
- The Color panel exposes usable swatches and current-color feedback; Color Picker updates color only, Brush Picker updates brush identity while preserving current color and normalized size, and Dropper copies stroke color, brush, and size.
...
- Full `.tilt` save/load/catalog parity, full layer management, selection/pin/snip/join/repaint/recolor/rebrush, Sketchbook and Settings alternate panels, floating panel attach/detach lifecycle, reference image Dropper sampling if it misses the Phase A timebox, full widget/stencil/tape/grid/lazy/mirror semantics, advanced local export formats, and local playback.

Out of scope for this port plan:

- Multiplayer, cloud storage, cloud-powered sharing, account-backed services, provider-hosted catalogs, networked collaboration, and multi-user editing.
...
- Mapped brushes carry source pressure-size and pressure-opacity ranges through runtime stroke geometry and GLB export; the default Light brush uses `[0.15,1]` pressure size and `[0.5,1]` pressure opacity so low-pressure Draw strokes are thinner and less opaque while Straightedge/Line remains intentionally full pressure until Open Brush parity says otherwise.
...
- Panel-focus suppression must be explicit in app/UI state: Draw, Eraser, and Picker actions blocked by an active Brush-ray intersection with a visible focusable panel set temporary `draw-panel-focus`, `erase-panel-focus`, or `pick-panel-focus` status and clear back to the active tool's ready status once the Brush ray leaves the panel.
...
- Unit: brush-size range conversion, mapped brush pressure-size and pressure-opacity ranges, upstream Light default size, picked-size propagation, straightedge thickness parity, eraser hit/miss tests, and transformed-stroke eraser regressions.
- Unit: generated geometry applies brush pressure opacity to vertex alpha for ribbon/emissive, tube, particle, and GLB export paths without changing stroke color metadata.
...
- Unit: eraser hit selection covers active-layer/finalized/visible/render-visible gates and verifies generated geometry results take precedence over fallback AABB bounds.
- Unit: tool pose math covers local `-Z` Brush-tool forward offset, clamping, and known-orientation world offset so visible cursor position and Eraser/Dropper/Pickers use the same controller-forward convention.
...
- Runtime E2E: verify cursor-center and erase-center pose parity by applying a known Brush-hand grip orientation, querying the cursor transform, pressing Activate over a stroke, and confirming the stroke erase center follows the same local forward axis as the visible cursor.
...
- Runtime E2E: aim the active Brush ray at a visible hand-attached panel while holding Draw, Eraser, and Picker triggers; verify temporary panel-focus statuses, visible Brush-panel feedback, no unintended stroke/pick/erase side effects, and status restoration when the ray leaves the panel.
...
- Runtime E2E panel-focus feedback regression: while aiming the Brush ray at a visible wand panel, verify `OpenBrushAppState.toolStatus` exposes `draw-panel-focus`, `erase-panel-focus`, or `pick-panel-focus`, the hand-attached Brush panel shows a panel-focus label, and moving the Brush ray away restores the normal ready status without reload.
