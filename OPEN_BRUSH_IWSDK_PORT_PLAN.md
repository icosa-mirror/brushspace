...
- Tool-spec audit queue: recheck every tool's source spec before declaring parity. Draw and Straightedge/Line startup thickness now has a concrete target and managed-runtime evidence via the upstream Light brush; remaining priority checks are brush-size controls, eraser visual/radius/intersection behavior, picker/dropper size copying, straightedge as global mode with parametric guide behavior, and any panel-focus conditions that can make eraser appear broken.
...
- Symmetry and straightedge generate expected grouped strokes with correct metadata; Straight Edge must behave as a FreePaint mode/toggle and produce reference-compatible parametric line control points instead of only two endpoints.
