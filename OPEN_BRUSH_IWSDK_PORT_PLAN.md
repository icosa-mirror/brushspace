...
- The renewed tool-spec audit remains active. The latest sidecar pass confirmed upstream Draw uses trigger ratio every frame, while IWSDK `InputCommandSystem` still falls back to pressure `1` for XR pressed-without-value, keyboard paint, and browser pointer paint; Straightedge/Line still intentionally matches upstream `LineCreator` pressure `1`. This means a full-pressure Draw or Line can look thicker than a light analog Draw stroke even with the correct `BrushSize01=0.5`. Next thickness work should instrument pressure values in managed XR and decide whether to preserve analog-only pressure, introduce a digital-input pressure default, or recalibrate live render width without breaking upstream Line semantics.
