...
- Managed runtime checks on browser generation 366 confirmed the hand-attached Tools panel now exposes and wires Mirror, Grid, Lazy, Tape, and Stencil in addition to Draw/Line/Eraser/Color/Brush/Dropper/Undo/Redo. Using the managed right-controller ray against the left/wand-attached panel, selecting `tool-mirror`, `tool-grid-snap`, `tool-lazy-input`, `tool-tape`, and `tool-stencil` changed `OpenBrushAppState.activeTool` to `mirror`, `grid-snap`, `lazy-input`, `tape`, and `stencil` respectively, with fresh warn/error console logs empty. The wand eraser selector is now labeled `Eraser` to distinguish the held eraser tool from the browser/debug fallback's one-shot `Erase Target` command.
...
- Hand-attached UI: core brush/color/tool controls belong on off-hand/wand-attached spatial panels with brush/wand role semantics. The consolidated panel remains a browser/debug fallback, not the target XR UX. The current XR wand panels are not parity yet: they cover Draw/Line/Eraser, Mirror, Grid, Lazy, Tape, Stencil, Color Picker, Brush Picker, Dropper, stroke undo/redo requests, brush prev/next/size, and a few color swatches. Missing XR controls include dropper image-widget sampling, layers, selection, settings, save/load/export/playback, paging/popups, direct touch, richer palette controls, and the reference-specific visuals/semantics behind the placeholder Mirror/Grid/Lazy/Tape/Stencil descriptors.
