...
- Eraser sizing has been partially corrected: the IWSDK eraser uses the upstream `StrokeModificationTool` range midpoint (`[0.1, 0.3]` -> `0.2`) instead of the previous `0.08` radius. The runtime now creates an `OpenBrushEraserCursor` entity and applies the Open Brush scene's eraser `m_PointerForwardOffset=0.05` so the visible cursor and erase center sit in front of the brush-hand ray. Remaining eraser parity still requires resizable radius controls, hot/cold material polish, audio/haptics, and tighter geometry/widget intersection.
