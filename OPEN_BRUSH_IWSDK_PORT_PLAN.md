...
- Eraser deletes only intersected visible/unlocked strokes or widgets through live brush-controller interaction; translated stroke-object offsets are honored during hit tests; it has visible/resizable tool radius behavior equivalent to Open Brush `StrokeModificationTool`; and screen-space or hand-attached panel hover cannot globally block XR erasing unless the active pointer ray is actually over the panel. Full parity still requires rotation/scale, layer/canvas transform, generated-geometry, and widget intersection coverage. Eraser and selection operations are undoable and do not leak geometry.
