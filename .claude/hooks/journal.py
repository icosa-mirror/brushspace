...
# The final assistant message can hit the transcript file a beat after the
# Stop hook fires. Re-read until the file stops growing (bounded).
SETTLE_ROUNDS = 2
SETTLE_DELAY = 0.25
    ...
    for entry in new_entries:
        ...
        if etype == "user":
            ...
            if isinstance(content, str):
                ...
                # User interjections (interrupts, pasted text/images) arrive as
                # list-form content. Only treat text as user input when the
                # entry carries no tool_results — reminder text injected next
                # to tool_results is harness noise, not the user speaking.
                has_tool_result = any(
                ...
                for block in content:
                    ...
                    if btype == "text" and not has_tool_result:
                        t = block.get("text", "")
                        if t.lstrip().startswith("<system-reminder>"):
                            continue
                        text_parts.append(t)
                        continue
                    if btype == "image" and not has_tool_result:
                        has_user_image = True
                        continue
                ...
                if text_parts or has_user_image:
        ...
        if etype == "attachment":
            # Messages sent while Claude is working never become type=user
            # entries — they are delivered as queued_command attachments.
            att = entry.get("attachment") or {}
            if att.get("type") == "queued_command":
                origin = (att.get("origin") or {}).get("kind")
                text, was_truncated = truncate_text(att.get("prompt", "") or "")
                evt = {
...
def _run():
    ...
    session_end = payload.get("hook_event_name") == "SessionEnd"
    ...
    for _ in range(SETTLE_ROUNDS):
        ...
        more, new_offset, _ = parse_transcript_slice(transcript_path, new_offset)
        if not more:
            break
        new_entries.extend(more)
    ...
    if session_end and state.get("last_decision") == "allowed":
        # The turn counter was already bumped at the last allowed stop; any
        # late-flushed tail entries still belong to the turn that just ended.
    ...
    if session_end:
        decision = {"action": "allowed", "reason": "session_end"}
    ...
    # again); an allowed stop ends the turn. SessionEnd is a final sweep and
    # leaves the turn counter alone.
    if not session_end:
